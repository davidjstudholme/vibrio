       refdir = ../06_ref  # directory where reference files are located
      workdir = ../07_workdir # directory where contigs/reads files are located and output is stored

    reference = 1  # 0:pick a random reference; 1:use given reference; 2: use ANI based reference
      reffile = V._alginolyticus_NBRC_15630_T.fasta  # reference filename 
